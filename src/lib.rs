...
use config::{
    app_state::AppState,
    db::{init_db, init_redis_session_store},
    jwt::JwtManager,
};
...
pub async fn start_server() {
    ...
    let session_store =
        init_redis_session_store(env::var("REDIS_URL").expect("REDIS URL must be set"));

    let app_state = Arc::new(AppState::new(db_pool, session_store));
